/* Context Debugger view — buildContext receipts rendered in the dashboard. */
...
async function runDebug() {
    ...
    document.getElementById('debug-meta').textContent = receipt
        ? `${receipt.totals.selected}/${receipt.totals.candidates} candidates selected · ` +
          `${receipt.totals.usedChars} chars · asOf ${receipt.asOf}`
        : '';
    ...
    for (const candidate of receipt?.candidates || []) {
        ...
        row.className = candidate.decision === 'selected' ? 'row-selected' : 'row-skipped';
        const mark = document.createElement('td');
        mark.textContent = candidate.decision === 'selected' ? '✓' : '–';
        const type = cell(candidate.type);
        const scopeCell = cell(candidate.scope);
        const trustCell = cell(`${candidate.sensitivity} / ${candidate.sourceTrust}`);
        const matchParts = [
        ...
        const chars = cell(String(candidate.chars));
        const decision = cell(`${candidate.decision} (${candidate.reason})`);
