...
import { getLanguage } from '../lang/languages.js';

export function generateSoulMd(personality, fingerprint, userInfo = {}) {
    const name = userInfo.name || 'User';
    const contacts = userInfo.contacts || {};
    // Language-appropriate fallback phrases (English when the language is unknown/mixed).
    const fallback = getLanguage(personality.primaryLanguage).fallback;
...
- Deflect topics: Deep politics, religion (say "${fallback.deflect}")
- When unsure: Say "${fallback.unsure}" (don't make up answers)
